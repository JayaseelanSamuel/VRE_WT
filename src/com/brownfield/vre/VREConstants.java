...
public class VREConstants {
	...
	/** The email group. */
	public static String GENERAL_NOTIFICATION_GROUP = "GENERAL";
	...
	public static int CONCURRENT_PIPESIM_LICENCES = 1;
	 ...
	 */
	public static final String WELL_TEST_CALIBRATE_QUERY = "SELECT WELL_TEST_ID,STRING_ID, QL1, WHP1, TEST_START_DATE, TEST_END_DATE, EFFECTIVE_DATE,IS_CALIBRATED, TEST_WATER_CUT, WATER_CUT_LAB, PIPESIM_MODEL_LOC, ROW_CHANGED_BY, ROW_CHANGED_DATE, RECORDED_DATE FROM(" 
			+ "SELECT WT.WELL_TEST_ID, WT.STRING_ID, QL1, WHP1, TEST_START_DATE, TEST_END_DATE, CAST(TEST_END_DATE AS DATE) AS EFFECTIVE_DATE, IS_CALIBRATED, TEST_WATER_CUT, SM.PIPESIM_MODEL_LOC, WT.ROW_CHANGED_BY, WT.ROW_CHANGED_DATE, DD.RECORDED_DATE, DD.WATER_CUT_LAB,"
			+ "RANK() OVER(PARTITION BY WELL_TEST_ID,WT.STRING_ID ORDER BY WELL_TEST_ID,WT.STRING_ID,DD.RECORDED_DATE DESC) TOPVAL "
			+ "FROM WELL_TEST WT LEFT OUTER JOIN STRING_METADATA SM ON WT.STRING_ID = SM.STRING_ID "
			+ "LEFT JOIN DAILY_ALLOCATED_DATA DD ON DD.STRING_ID=WT.STRING_ID AND DD.RECORDED_DATE<=WT.TEST_END_DATE AND WATER_CUT_LAB>=0 "  
			+ "WHERE TEST_TYPE = 'SR' AND VRE_FLAG = 1 AND IS_CALIBRATED IS NULL AND SM.PIPESIM_MODEL_LOC IS NOT NULL)T WHERE T.TOPVAL=1 ";
	
	//Jay changes
	public static final String LAST_WELL_TEST_CALIBRATE_QUERY = "SELECT STRING_ID,STRING_TYPE,UWI,STRING_NAME,STRING_CATEGORY_ID,CURRENT_STATUS,PIPESIM_MODEL_LOC,WELL_TEST_ID,TEST_START_DATE,TEST_END_DATE,QL1,WHP1,EFFECTIVE_DATE,IS_CALIBRATED,TEST_WATER_CUT,WATER_CUT_LAB,ROW_CHANGED_BY,ROW_CHANGED_DATE FROM("
			+ "SELECT SS.STRING_ID,SS.STRING_TYPE,SS.UWI,SS.STRING_NAME,SS.STRING_CATEGORY_ID,SS.CURRENT_STATUS,SM.PIPESIM_MODEL_LOC,WT.RN,WELL_TEST_ID,TEST_START_DATE,TEST_END_DATE,QL1,WHP1,EFFECTIVE_DATE,DD.RDATE,IS_CALIBRATED,TEST_WATER_CUT,DD.WATER_CUT_LAB"  
			+ ",WT.ROW_CHANGED_BY,WT.ROW_CHANGED_DATE"
			+ ",ROW_NUMBER() OVER( PARTITION BY SS.STRING_ID ORDER BY SS.STRING_ID,EFFECTIVE_DATE,DD.RDATE DESC) RNX " 
			+ "FROM STRING SS "
			+ "INNER JOIN STRING_METADATA SM ON SS.STRING_ID = SM.STRING_ID AND SM.PIPESIM_MODEL_LOC IS NOT NULL "
			+ "INNER JOIN (SELECT ROW_NUMBER() OVER( PARTITION BY STRING_ID ORDER BY STRING_ID,TEST_END_DATE DESC) RN,WELL_TEST_ID,STRING_ID,TEST_START_DATE,TEST_END_DATE,QL1,WHP1,CAST(TEST_END_DATE AS DATE) AS EFFECTIVE_DATE,IS_CALIBRATED,TEST_WATER_CUT,ROW_CHANGED_BY,ROW_CHANGED_DATE "
			+ "FROM WELL_TEST WHERE (TEST_TYPE = 'FT') AND CAST(TEST_END_DATE AS DATE) <= cast(getdate() as date)) WT on SS.string_id=WT.STRING_ID AND RN=1 " 
			+ "INNER JOIN (SELECT STRING_ID,WATER_CUT_LAB,CAST(RECORDED_DATE AS DATE) AS RDATE FROM DAILY_ALLOCATED_DATA WHERE WATER_CUT_LAB>=0) DD ON DD.STRING_ID=SS.STRING_ID AND RDATE<=WT.EFFECTIVE_DATE " 
			+ ")X WHERE RNX=1";
	
	//Jay changes
		public static final String LAST_WELL_TEST_CALIBRATE_QUERY_WITHSTRING = "SELECT STRING_ID,STRING_TYPE,UWI,STRING_NAME,STRING_CATEGORY_ID,CURRENT_STATUS,PIPESIM_MODEL_LOC,WELL_TEST_ID,TEST_START_DATE,TEST_END_DATE,QL1,WHP1,EFFECTIVE_DATE,IS_CALIBRATED,TEST_WATER_CUT,WATER_CUT_LAB,ROW_CHANGED_BY,ROW_CHANGED_DATE FROM("
				+ "SELECT SS.STRING_ID,SS.STRING_TYPE,SS.UWI,SS.STRING_NAME,SS.STRING_CATEGORY_ID,SS.CURRENT_STATUS,SM.PIPESIM_MODEL_LOC,WT.RN,WELL_TEST_ID,TEST_START_DATE,TEST_END_DATE,QL1,WHP1,EFFECTIVE_DATE,DD.RDATE,IS_CALIBRATED,TEST_WATER_CUT,DD.WATER_CUT_LAB"  
				+ ",WT.ROW_CHANGED_BY,WT.ROW_CHANGED_DATE"
				+ ",ROW_NUMBER() OVER( PARTITION BY SS.STRING_ID ORDER BY SS.STRING_ID,EFFECTIVE_DATE,DD.RDATE DESC) RNX " 
				+ "FROM STRING SS "
				...
				+ "FROM WELL_TEST WHERE (TEST_TYPE = 'FT') AND CAST(TEST_END_DATE AS DATE) <= cast(getdate() as date)) WT on SS.string_id=WT.STRING_ID AND RN=1 " 
				+ "INNER JOIN (SELECT STRING_ID,WATER_CUT_LAB,CAST(RECORDED_DATE AS DATE) AS RDATE FROM DAILY_ALLOCATED_DATA WHERE WATER_CUT_LAB>=0) DD ON DD.STRING_ID=SS.STRING_ID AND RDATE<=WT.EFFECTIVE_DATE " 
				+ ")X WHERE RNX=1 and STRING_ID in(7,519,520,512)";
	...
	public static final String WHP_AT_TECH_RATE_QUERY = "SELECT STRING_TARGET_RATE_ID, SMTR.STRING_ID, TECHNICAL_RATE, WELLHEAD_PRESSURE_AT_TECH_RATE, MAX_FLOW_RATE_PRESSURE, SMTR.RECORDED_DATE, SM.PIPESIM_MODEL_LOC,  "
	...
	public static final String INSERT_TARGET_RATE_QUERY = "INSERT INTO STRING_MONTHLY_TARGET_RATE (STRING_ID, RECORDED_DATE, TECHNICAL_RATE, MAX_FLOW_RATE_PRESSURE,ROW_CHANGED_BY) "
			+ " VALUES (?, ?, ?, ?) ";
	
	public static final String NOTIFICATION_EMAIL_QUERY = "select EMAILADDRESS from USERTYPE where USERTYPE=? ";
	...
	/** The Constant MODEL_ALERT_MESSAGE. */
	public static final String MODEL_ALERT_MESSAGE = "%s value (%s) for Well %S varies more than %s%% limit of previous day value %s at time %s";
	...
	public static String RECAL_WORKFLOW = "Recalibration Workflow";
	...
	/** The Constant PERCENT_LIMIT. */
	public static final String PERCENT_LIMIT = "PERCENT_LIMIT";
