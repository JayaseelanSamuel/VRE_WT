...
public class VREConstants {
	...
	/*public static final String VRE_DATASET_QUERY = "SELECT T.*, DAM.RECORDED_DATE, ISNULL(DAM.AVG_WHP, DD.WELLHEAD_PRESSURE) AS AVG_WHP, DD.WATER_CUT_LAB, "
			+ " DAM.AVG_DOWNHOLE_PRESSURE, DAM.AVG_GASLIFT_INJ_RATE, DAM.AVG_HEADER_PRESSURE, DD.CHOKE_SETTING "
			+ " FROM ( " + " SELECT S.STRING_ID, S.UWI, S.STRING_TYPE, SM.PIPESIM_MODEL_LOC, R.RESERVOIR_MODEL_LOC "
			...
			+ "	WHERE SM.PIPESIM_MODEL_LOC IS NOT NULL " + " AND P.PLATFORM_NAME IN ('IN') ) T "
			// AND TAG_WHP IS NOT NULL
	...
	public static final String VRE_DATASET_QUERY = "SELECT T.*, DAM.RECORDED_DATE, DAM.AVG_WHP AS AVG_WHP_ORI, DD.WELLHEAD_PRESSURE,  "
			+ " CASE WHEN (DAM.AVG_WHP IS NULL OR DAM.AVG_WHP = 0 OR T.TAG_WHP IS NULL OR WHP_ALERT IS NOT NULL) THEN DD.WELLHEAD_PRESSURE ELSE DAM.AVG_WHP END AS AVG_WHP, " 
	+ " DAM.AVG_HEADER_PRESSURE AS AVG_HEADER_PRESSURE_ORI, PHP.HEADER_PRESSURE, " 
	+ " CASE WHEN (DAM.AVG_HEADER_PRESSURE IS NULL OR DAM.AVG_HEADER_PRESSURE = 0 OR T.TAG_HEADER_PRESSURE IS NULL OR HEADER_PRESSURE_ALERT IS NOT NULL)  "
	+ " THEN PHP.HEADER_PRESSURE ELSE DAM.AVG_HEADER_PRESSURE END AS AVG_HEADER_PRESSURE, " 
	+ " T.TAG_WHP, T.TAG_HEADER_PRESSURE, " 
	+ " CASE WHEN (DAM.AVG_WHP IS NULL OR DAM.AVG_WHP = 0 OR T.TAG_WHP IS NULL OR WHP_ALERT IS NOT NULL OR T.TAG_HEADER_PRESSURE IS NULL OR HEADER_PRESSURE_ALERT IS NOT NULL OR " 
	+ " ((DAM.AVG_HEADER_PRESSURE IS NULL OR DAM.AVG_HEADER_PRESSURE = 0) AND PHP.HEADER_PRESSURE IS NOT NULL)) THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS IS_SEABED, "
	...
	+ " DAM.AVG_DOWNHOLE_PRESSURE, VWAP.WHP_ALERT, VWAP.HEADER_PRESSURE_ALERT, DAM.AVG_GASLIFT_INJ_RATE, DD.CHOKE_SETTING, ISNULL(DD.WATER_CUT_LAB,0) AS WATER_CUT_LAB "
	+ " FROM ( " 
	...
	+ " ) AS T "
	+ " LEFT OUTER JOIN DAILY_AVERAGE_MEASUREMENT DAM ON DAM.STRING_ID = T.STRING_ID AND DAM.RECORDED_DATE = ? " 
	+ " LEFT OUTER JOIN DAILY_ALLOCATED_DATA DD ON DD.STRING_ID = T.STRING_ID AND DD.RECORDED_DATE = DAM.RECORDED_DATE " 
	+ " LEFT OUTER JOIN PLATFORM_HEADER_PRESSURE PHP ON PHP.PLATFORM_ID = T.PLATFORM_ID AND PHP.RECORDED_DATE = DAM.RECORDED_DATE "
	+ " LEFT OUTER JOIN VRE_WELL_ALARMS_PIVOT_DAILY_V AS VWAP ON VWAP.STRING_ID = T.STRING_ID AND VWAP.RECORDED_DATE = DAM.RECORDED_DATE "
	+ " WHERE ((DAM.AVG_WHP IS NOT NULL AND DAM.AVG_WHP <> 0) OR (DD.WELLHEAD_PRESSURE IS NOT NULL AND DD.WELLHEAD_PRESSURE <> 0)) " ;

	/** The Constant VRE_DURATION_QUERY. */
	/*public static final String VRE_DURATION_QUERY = "SELECT T.*, DAM.RECORDED_DATE, ISNULL(DAM.AVG_WHP, DD.WELLHEAD_PRESSURE) AS AVG_WHP, DD.WATER_CUT_LAB, DAM.AVG_DOWNHOLE_PRESSURE, "
			...
			+ "	LEFT OUTER JOIN SECTOR SC ON SC.SECTOR_ID = SSA.SECTOR_ID  "
			+ "	LEFT OUTER JOIN RESERVOIR R ON R.RESERVOIR_ID = SC.RESERVOIR_ID  "
			+ "	WHERE SM.PIPESIM_MODEL_LOC IS NOT NULL " + " ) T  "
			// AND TAG_WHP IS NOT NULL
	...
	+ " DAM.AVG_DOWNHOLE_PRESSURE, VWAP.WHP_ALERT, VWAP.HEADER_PRESSURE_ALERT, DAM.AVG_GASLIFT_INJ_RATE, DD.CHOKE_SETTING, ISNULL(VR.WATER_CUT,0) AS WATER_CUT "
	+ " FROM ( " 
	+ "		SELECT DISTINCT S.STRING_ID, S.UWI, S.STRING_TYPE, SM.TAG_WHP, P.TAG_HEADER_PRESSURE, SM.PIPESIM_MODEL_LOC, R.RESERVOIR_MODEL_LOC, P.PLATFORM_ID "
		+ " FROM STRING S LEFT OUTER JOIN STRING_METADATA SM ON SM.STRING_ID = S.STRING_ID "
		+ " LEFT OUTER JOIN WELL W ON W.UWI = S.UWI "
		...
		+ " WHERE SM.PIPESIM_MODEL_LOC IS NOT NULL " 
	+ " ) AS T "
	+ " LEFT OUTER JOIN DAILY_AVERAGE_MEASUREMENT DAM ON DAM.STRING_ID = T.STRING_ID " 
	+ " LEFT OUTER JOIN DAILY_ALLOCATED_DATA DD ON DD.STRING_ID = T.STRING_ID AND DD.RECORDED_DATE = DAM.RECORDED_DATE " 
	+ " LEFT OUTER JOIN PLATFORM_HEADER_PRESSURE PHP ON PHP.PLATFORM_ID = T.PLATFORM_ID AND PHP.RECORDED_DATE = DAM.RECORDED_DATE "
	+ " LEFT OUTER JOIN WELL_TEST WT ON WT.STRING_ID = T.STRING_ID AND DAM.RECORDED_DATE = CAST(WT.TEST_END_DATE AS DATE) AND WT.VRE_FLAG = 'TRUE' "
	+ " LEFT OUTER JOIN VIRTUAL_RATE_ESTIMATION VR ON VR.STRING_ID = T.STRING_ID AND VR.RECORDED_DATE = DAM.RECORDED_DATE "
	+ " LEFT OUTER JOIN VRE_WELL_ALARMS_PIVOT_DAILY_V AS VWAP ON VWAP.STRING_ID = T.STRING_ID AND VWAP.RECORDED_DATE = DAM.RECORDED_DATE "
	+ " WHERE T.STRING_ID = ? AND DAM.RECORDED_DATE BETWEEN ? AND ? AND "
	...
	public static final String VRE_TABLE_SELECT_QUERY = "SELECT STRING_ID, RECORDED_DATE, VRE1, VRE2, VRE3, VRE4, VRE5, VRE6, "
			...
			+ " FROM VIRTUAL_RATE_ESTIMATION "
			+ " WHERE RECORDED_DATE = ? AND STRING_ID = ? ";
	...
	public static final String ARG_RECALIBRATE_HIGH = "-error";
