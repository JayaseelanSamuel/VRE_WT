...
public class VREConstants {
	...
	public static int MODEL_PREDICTION_THREAD_COUNT = 2;
	...
	public static final String SELECT_LATEST_WHP_QUERY = "SELECT * FROM ( " + 
			" SELECT S.STRING_ID, RT.RECORDED_DATE, TAGQUALITY, S.TAG_WHP, DD.WELLHEAD_PRESSURE, TAGRAWVALUE, " + 
			" CASE WHEN S.TAG_WHP IS NULL THEN DD.WELLHEAD_PRESSURE ELSE ISNULL(TAGRAWVALUE, DD.WELLHEAD_PRESSURE) END AS WHP,  " + 
			" ISNULL(DD.WATER_CUT_LAB, 0) AS WATER_CUT_LAB  " + 
			" FROM (  " + 
			"	SELECT DISTINCT S.STRING_ID, S.UWI, S.STRING_TYPE, S.STRING_CATEGORY_ID, SM.TAG_WHP, P.TAG_HEADER_PRESSURE, SM.PIPESIM_MODEL_LOC,  " + 
			"	R.RESERVOIR_MODEL_LOC, R.RESERVOIR_NAME, P.PLATFORM_ID, P.PLATFORM_NAME " + 
			"	FROM STRING S LEFT OUTER JOIN STRING_METADATA SM ON SM.STRING_ID = S.STRING_ID " + 
			...
			"	LEFT OUTER JOIN SECTOR SC ON SC.SECTOR_ID = SSA.SECTOR_ID " + 
			"	LEFT OUTER JOIN RESERVOIR R ON R.RESERVOIR_ID = SC.RESERVOIR_ID " + 
			"	WHERE SM.PIPESIM_MODEL_LOC IS NOT NULL AND SM.IS_ACTIVE = 1 " + 
			" ) AS S " + 
			" LEFT OUTER JOIN DAILY_ALLOCATED_DATA DD ON DD.STRING_ID = S.STRING_ID AND DD.RECORDED_DATE = ? " + 
			" LEFT OUTER JOIN  " + 
			" ( " + 
			"	SELECT STRING_ID, MAX(RECORDED_DATE) AS REC_DATE " + 
			"	FROM REAL_TIME_DATA  " + 
			"	WHERE TAG_TYPE_ID = 1 " + 
			"	GROUP BY STRING_ID " + 
			" ) B  " + 
